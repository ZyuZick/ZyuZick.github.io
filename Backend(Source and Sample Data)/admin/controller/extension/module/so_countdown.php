...
class ControllerExtensionModuleSoCountdown extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_countdown/edit', 'token=' . $this->session->data['token'] . '&id='.$new_id, 'SSL'));
			}else {
				$this->response->redirect($this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . $url, true));
			}
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('extension/module/so_countdown/edit', 'token=' . $this->session->data['token'] . '&id='.$this->request->get['id'], 'SSL'));
			}else {
				$this->response->redirect($this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . $url, true));
			}
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('extension/module/so_countdown/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('extension/module/so_countdown/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			if (count($store_id)) {
			  foreach ($store_id as $_id) {
				if ($_id['store_id'] != 0) {
				 if( $this->model_setting_store->getStore($_id['store_id']) !=null){
			...
			$data['lists'][] = array(
				...
				'edit'        	=> $this->url->link('extension/module/so_countdown/edit', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, true),
				'delete'      	=> $this->url->link('catalog/category/delete', 'token=' . $this->session->data['token'] . '&id=' . $result['id'] . $url, true),
				'stores'		=> $stores
			);
		}
		...
		$data['sort_name'] = $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}
		...
		if (!isset($this->request->get['id'])) {
			$data['action'] = $this->url->link('extension/module/so_countdown/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('extension/module/so_countdown/edit', 'token=' . $this->session->data['token'] . '&id=' . $this->request->get['id'] . $url, true);
		}
		
		$data['cancel'] = $this->url->link('extension/module/so_countdown', 'token=' . $this->session->data['token'], true);
    ...
    public function install() {
        ...
        $this->model_extension_module->addModule('so_countdown', $data);
